...
export const determineMainType = (scores: ICRUScores): MainType => {
  ...
  switch (highestKey) {
    case 'R': return MainType.REVOLUTIONER;
    case 'E': return MainType.EMOTIONER;
    case 'A': return MainType.ACTIONER;
    case 'D': return MainType.DETECTOR;
    default: throw new Error('Invalid scores');
  }
};
...
export const getICRUCode = (scores: ICRUScores): ICRUCode => {
  ...
  const allOthersVeryLow = others.every(
    ([_, score]) => getPreferenceLevel(score) === PreferenceLevel.VERY_LOW
  );
